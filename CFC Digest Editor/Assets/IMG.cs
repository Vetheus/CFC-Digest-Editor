...
namespace CFC_Digest_Editor.Racjin.Assets
{
    ...
    public class IMG
    {
        ...
        public class IndexBlock
        {
            ...
            public ushort ClutIndex;
            public bool useTexture, useClut;
            public int Reserved;
            ...
            {
                TexIndex = (ushort)input.ReadUInt(0, 16),
                ClutIndex = (ushort)input.ReadUInt(8, 16),
                useTexture = Convert.ToBoolean(input[0xA]),
                useClut = Convert.ToBoolean(input[0xB]),
                Reserved = (int)input.ReadUInt(0xC, 32),
            ...
            {
                var list = new List<byte>();
                list.AddRange(BitConverter.GetBytes((UInt16)TexIndex));
                list.AddRange(Enumerable.Range(0, 6).Select(x => (byte)0xFF));
                list.AddRange(BitConverter.GetBytes((UInt16)ClutIndex));
                list.Add(Convert.ToByte(useTexture));
                list.Add(Convert.ToByte(useClut));
                list.AddRange(BitConverter.GetBytes((UInt32)Reserved));
        ...
        public IMG TryRead(byte[] input, PictureBox pictureBox)
        {
            var img = new IMG(_main);

            if (input.ReadUInt(0x10, 32) != 0x8004)
                return null;
        ...
        public IMG Read(byte[] input, PictureBox pictureBox)
        {
            ...
            foreach (var entry in img.Indexes)
            {
                ...
                int texOffset = img.Blocks[entry.TexIndex].BlockOffset
                    + 0x10 + (img.Count * 0x50) + (img.Count * 0x14) + (0x10 * entry.TexIndex);

                int texSize = (entry.Width * entry.Height) / img.Blocks[entry.ClutIndex].Bpp;

                int clutOffset = img.Blocks[entry.ClutIndex].BlockOffset
                    + 0x10 + (img.Count * 0x50) + (img.Count * 0x14) + (0x10 + (0x10 * (entry.ClutIndex-1)));
                ...
                if (texSize < (clutOffset - texOffset))
                {
                    ...
                    width = img.Blocks[entry.TexIndex].Width + img.Blocks[entry.TexIndex].Height2;
                    height = img.Blocks[entry.TexIndex].Height * 4;
                    wuznws = width;
                    huznws = img.Blocks[entry.TexIndex].Width + img.Blocks[entry.TexIndex].Height;
                }

                
                int clutSize = img.Blocks[entry.ClutIndex].Bpp == 2 ? 0x40 : 0x400;
                ...
                var newImage = new Image()
                {
                    ...
                    Width = width,
                    Height = height,
                    Bpp = img.Blocks[entry.ClutIndex].Bpp == 2 ? 4 : 8,
                    
                    Wunswizz = wuznws,
                    Hunswizz = huznws,

                    Width_A = img.Blocks[entry.TexIndex].Width,
                    Height_A = img.Blocks[entry.TexIndex].Height,
                    Width_B = img.Blocks[entry.TexIndex].Height2,

                    CLT_Width_A = img.Blocks[entry.ClutIndex].Width,
                    CLT_Height_A = img.Blocks[entry.ClutIndex].Height,
                    CLT_Width_B = img.Blocks[entry.ClutIndex].Height2,
        ...
        public byte[] RebuildIMG(int bpp)
        {
            ...
            for (int i =0; i< Indexes.Count; i++)
            {
                ...
                Indexes[i].TexIndex = (ushort)k;
                Indexes[i].ClutIndex = (ushort)(k + 1);
        ...
        public byte[] GetImage(int Index, out System.Drawing.Image mage)
        {
            ...
            if (cores.Length <= 256 && cores.Length > 64)
            {
                ...
                ImageDecoderIndexed imageDecoder = new ImageDecoderIndexed(unswizzledPixelData, width, height, 
                    IndexCodec.FromNumberOfColors(cores.Length, Endianess.LittleEndian), cores);
            ...
            if (cores.Length <= 16)
            {
                ...
                ImageDecoderIndexed imageDecoder = new ImageDecoderIndexed(
                    unswizz,
                    width, height,
                    IndexCodec.FromNumberOfColors(16, Endianess.LittleEndian), cores);
                image = imageDecoder.DecodeImage();
                mage = image;
        ...
        public static byte[] ConvertPS2EA4bit(byte[] imageData, int imgWidth, int imgHeight, int bpp, bool swizzleFlag)
        {
            ...
            if (swizzleFlag)
                ...
                catch
                {
                    ...
                    MessageBox.Show("Error while swizzling image!", _main.title, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            else
                try
                {                    
                ...
                {
                    convertedData = imageData;
                    MessageBox.Show("Error while unswizzling image!", _main.title, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
