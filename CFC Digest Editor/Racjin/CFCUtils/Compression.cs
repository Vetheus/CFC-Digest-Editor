...
namespace CFC_Digest_Editor.Racjin.CFCUtils
{
    public static class Compression
    {
        ...
        public static byte[] Decompress(byte[] buffer, int decompressedSize)
        {
            ...
            for (uint index = 0; index < decompressedSize; index++)
            {
        ...
        public static byte[] Compress(byte[] buffer, CFCGEN Gen)
        {
            ...
            while (index < buffer.Length)
            {
                ...
                if (Gen == CFCGEN.CDDATA && frequencies[lastEnc] == 256)
                {
                    frequencies[lastEnc] = 0x00;
                }
                ...
                for (byte freq = 0; freq < positionsToCheck; freq++)
                {
                    ...
                    byte matched = 0;
                    var maxLength = (uint)(index + 8 < buffer.Length ? 8 : buffer.Length - index);
