...
namespace CFC_Digest_Editor
{
    ...
    public static class ADPCM
    {
        ...
        public static byte[] ToPCMStereo(byte[] data, int size, int interleave)
        {
            ...
            int blockCount = size / 16; // Total de blocos de 16 bytes
            int interleave_blocks = interleave / 16;

            for (int i = 0; i < blockCount / 2; ++i)
            {
                int block_in_interleave = i % interleave_blocks;
                int interleave_group = i / interleave_blocks;

                int left_offset = interleave_group * interleave * 2 + block_in_interleave * 16;
                int right_offset = left_offset + interleave;

                if (right_offset + 16 > data.Length)
                    break;

                byte[] line_l = new byte[16];
                byte[] line_r = new byte[16];
                Array.Copy(data, left_offset, line_l, 0, 16);
                Array.Copy(data, right_offset, line_r, 0, 16);
                ...
                {
                    pcm_data.Add(l[j * 2]);
                    pcm_data.Add(l[j * 2 + 1]);
                    pcm_data.Add(r[j * 2]);
                    pcm_data.Add(r[j * 2 + 1]);
                }
